...
description: "集成 17 个搜索引擎（8 国内 + 9 国际），无需 API Multi search engine integration with 16 engines (7 CN + 9 Global). Supports advanced search operators, time filters, site search, privacy engines, and WolframAlpha knowledge queries. No API keys required."
user-invocable: true
---
...
- **执行通道**：本技能靠网页抓取跑搜索，DSH 下用 `web_fetch`（拉取页面）、`web_search`（补充/兜底）。上游写的 Browser / fetch 类工具名按此对应。
- **限速**：批次之间保持 1-2 秒间隔（`pwsh`：`Start-Sleep -Seconds 2`），每批 3-4 个引擎串行，不要并发猛拉。
- **被挡时**（403/429）：先取引擎首页拿新 cookie 再重试，与上游第 4 步一致。
- **资源位置**：`references/advanced-search.md`（国内引擎指南）、`references/international-search.md`（国际引擎指南）相对**本技能目录**解析；`config.json` 存引擎清单；`CHANGELOG.md` 是版本历史。
- **默认路由提示**：DSH 原生已有 `web_search`（Tavily）。简单查询直接用原生工具；本技能的价值在于**多引擎交叉取证**（尤其国内引擎 + 隐私引擎 + WolframAlpha 知识查询），或原生工具结果不足时扩大覆盖。
