...
> **DSH environment note:** the original text targets MiMo's bundled runtime (`MIMO_PYTHON` / `MIMO_SOFFICE` / `MIMO_NODE` / `MIMO_NODE_MODULES`). DSH has no such runtime — use the system `python` or `uv run` for Python; install LibreOffice yourself for `soffice`; for the PptxGenJS authoring path run `npm install pptxgenjs react react-dom sharp react-icons mathjax-full` yourself. Replace every `"$MIMO_PYTHON"` with `python`.
...
(`scripts/soffice_bridge.py`) — copy the whole `scripts/` directory together. Read the top
...
Note: `tavily_extract` returns text/markdown/html only and cannot deliver binary
...
- **Using `tavily_extract` to grab image bytes.** `tavily_extract` returns text only.
