# video-post-production install script for Windows (PowerShell).
#
...
#   $env:VIDEO_POST_FFMPEG_INSTALLER  -> winget (default) | choco | scoop | skip
#   $env:VIDEO_POST_SKIP_PYTHON_CHECK -> 1 to skip python detection
...
function Install-Ffmpeg {
    $installer = $env:VIDEO_POST_FFMPEG_INSTALLER
    if (-not $installer) { $installer = 'winget' }

    switch ($installer) {
        'skip' {
            Write-Warning "VIDEO_POST_FFMPEG_INSTALLER=skip — assuming you'll install ffmpeg manually."
            return
        }
        'winget' {
            if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
                Write-Warning 'winget not found. Install via App Installer from the Microsoft Store, or set $env:VIDEO_POST_FFMPEG_INSTALLER="choco"/"scoop".'
        ...
        default {
            throw "Unsupported VIDEO_POST_FFMPEG_INSTALLER='$installer'. Use winget, choco, scoop, or skip."
        }
    }
}
...
function Test-PythonVersion {
    ...
    if ($major -lt 3 -or ($major -eq 3 -and $minor -lt 8)) {
        throw "Python $version detected at $($py.Source); video-post-production requires Python 3.8+."
    }
...
Write-Section 'video-post-production Windows installer'
...
if ($env:VIDEO_POST_SKIP_PYTHON_CHECK -ne '1') {
    Test-PythonVersion
}
...
----------------------------------------------------------------------------
video-post-production Windows dependencies are ready.
