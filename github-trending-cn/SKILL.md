...
# 本周热门，Top 25
python scripts/github_trending.py --period weekly --limit 25
...
- **解释器**：本机 `python3` 是 Microsoft Store 占位程序（静默无输出），一律用 `python`（实测 Python 3.13.5）。脚本零 pip 依赖，stdlib-only。
- **Token 注入**（提限额 10 → 30 次/分）：token 存在 `$env:USERPROFILE\.dsh\secrets\github.token`，调用前临时注入，不落进命令行历史：
...
- 也可以用 `--token` 显式传；不带 token 也能跑，只是限额低。
- 本机 token 实测：core 5000/h、search 30/min（账号 Practice019）。
- 脚本路径相对**本技能目录**：`scripts/github_trending.py`。
