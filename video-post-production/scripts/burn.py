...
stream is copied without re-encoding. A CJK font directory can be supplied
explicitly with ``--fonts-dir`` or through the ``VIDEO_FONTS_DIR``
environment variable.

Typical use: after ``merge.py`` has produced the merged ``final.mp4``, this
script writes ``final_subtitled.mp4`` next to it, in the voiceover's language.
...
def main() -> int:
    ...
    parser.add_argument(
        "--fonts-dir",
        default=os.environ.get("VIDEO_FONTS_DIR", ""),
        help="Directory containing subtitle fonts. Defaults to $VIDEO_FONTS_DIR when set; "
             "otherwise ffmpeg uses its own font configuration.",
    )
    ...
    fonts_dir_arg = args.fonts_dir or os.environ.get("VIDEO_FONTS_DIR", "")
    if fonts_dir_arg:
