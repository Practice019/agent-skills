...
> **DSH environment note:** the original text targets MiMo's bundled runtime (`MIMO_PYTHON` / `MIMO_QPDF`). DSH has no such runtime — use the system `python` or `uv run` instead. Replace every `"$MIMO_PYTHON"` with `python` and `"$MIMO_QPDF"` with `qpdf` (install qpdf yourself if the scripts need it).
