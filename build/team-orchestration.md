...
5. ★ 冲突扫描     本轮要派的 N 个任务，列出各自「预计改动文件/位置」并两两比对：
                  同一文件同一函数 → **合并**；同文件不同函数 → 并行但划定边界
                  无交集 → 安全并行
                  ⚠️ 不扫就派 = 运行时才撞冲突（见「任务队列与认领」的冲突检查）
6. 分配 + 扇出    给这 N 个任务逐个标记认领者，**同一轮发 N 个**：
                  subagent(run_in_background: false) × N（N 上限 5）
                  ⚠️ 同一轮发多个 = 真并行 + 全部返回后才继续
7. 收敛           默认(false) → 工具返回即已收敛，无需额外动作
                  用了(true)  → 无事后等待接口，结果靠推送（下一轮）
8. 更新队列       通过 → 标 done + commit hash；失败 → 回退到 pending
9. ★ 重算可认领   本轮标 done 后，**立刻重算可认领集**：
                  有新增解锁的 → **本轮直接追加派发**，不要等下一轮
                  仍不足 5 个且还有可认领 → 继续补派
10. 评审          命中触发条件（不可逆/安全/接口/需评审/有不确定）的任务：
                  任务移入 review/，派**另一个**子代理评审（见「评审门禁」）
                  不通过 → 回退 pending/ 带问题重派
11. 交棒          汇总产物、更新 goal 状态、决定下一轮
```

> ★ **第 2 步防的是最危险的静默失败**：崩溃后 `claimed/` 有孤儿、
> 但没有活着的子代理 —— 看起来像"在等"，实际永远等不到。
>
> ★ **第 5 步是派发的第一道闸门。** 冲突在**派发前**发现 = 一行合并；
> 在**运行时**发现 = 烧掉好几轮。（实证：三个任务同写一个装配文件 → 编译间歇红）
>
> ★ **第 9 步是依赖任务的关键。** 一轮派 N 个 → 其中一个完成 → 它解锁了下游任务
