---
name: web-search-exa
description: "基于 Exa 的神经网络搜索引擎，语义搜索、内容提取与深度研究 Neural web search, content extraction, company and people research, code search, and deep research via the Exa MCP server. Use when you need to: (1) search the web with semantic understanding — not just keywords, (2) find research papers, news, tweets, companies, or people, (3) extract clean content from URLs, (4) find semantically similar pages to a known URL, (5) get code examples and documentation, (6) run deep multi-step research with a report, (7) get a quick synthesized answer with citations. NOT for: local file operations, non-web tasks, or anything that doesn't involve web search or content retrieval."
user-invocable: true
---
...
**已配置**：Exa MCP server 已写入 DSH 的 MCP 配置（`$env:USERPROFILE\.dsh\mcp.json`），transport 为 `streamable-http`：

~~~text
https://mcp.exa.ai/mcp?exaApiKey=<已配置的 key>
~~~

- 生效方式：MCP 由 DSH 的技能与 MCP 管理器插件维护。新增/改动后需要在设置页「Web UI 插件 → 技能与 MCP」确认该 server 处于启用状态；已启用的 server 会真实连接并把工具注册成 `mcp__<server>__<tool>`。
- **工具名映射**：下文写的 `web_search_exa`、`crawling_exa` 等在 DSH 里实际是 `mcp__exa__web_search_exa`、`mcp__exa__crawling_exa` 这种带前缀的名字。用前先看当前会话可用工具列表，别按裸名调用。
- 若工具没出现：说明 MCP 尚未连接（未启用/未重启），此时先用原生 `web_search` / `web_fetch`，并告知用户 MCP 待启用。
- 本技能是**工具使用参考**（查询写法、参数、category 选择、token 效率），不包含可执行脚本。
