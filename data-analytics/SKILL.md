...
> **DSH runtime note:** DSH does not ship the connector/warehouse/browser integrations this bundle was written against (no MCP connector registry, no hosted widget runtime). Run in degraded mode: use whatever data access the host actually exposes (local files, SQL via Bash, `web_search`/`tavily_extract`), and say plainly which source was unavailable instead of pretending it worked.
