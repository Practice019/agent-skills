...
上游用 Playwright MCP 驱动浏览器；DSH 环境按下表替换，流程与产出格式不变：

| 上游写法 | DSH 对应 |
|---|---|
| `mcp__playwright__browser_navigate` / `click` / `type` / `snapshot` / `evaluate` 等 | 走 `browser-harness` 技能（CDP 直连 Chrome），按该技能手册执行导航、点击、取快照、跑 JS |
| `browser_tabs` list（检查现有 Tab） | `browser-harness` 的标签页操作 |
| `mcp__playwright__browser_close` | `browser-harness` 关闭页/断开连接 |
| `pkill -f "mcp-chrome"` | `pwsh`：`Get-Process chrome -ErrorAction SilentlyContinue \| Stop-Process`（只杀本项目用户数据目录起的实例时，先核对命令行） |
| `WebSearch` / `WebFetch` | `web_search` / `web_fetch`（补充用；多 AI 服务并行仍是主线） |
| Bash 系列（`mkdir`/`ls`/`mv`/`cp`/`sleep`/`date`） | `pwsh`（Windows：`New-Item` / `Get-ChildItem` / `Move-Item` / `Copy-Item` / `Start-Sleep` / `Get-Date`） |

注意：本机未安装 Playwright MCP，也没有对应的浏览器工具名；上面这些名字若直接当工具调用会失败。先确认 `browser-harness` 技能可用，再按它的手册开研究流程。若浏览器通道不可用，降级为 `web_search` + `web_fetch` 的多角度检索，并在报告里如实标注「未走多 AI 服务并行」。
