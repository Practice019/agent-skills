...
**开讨论之前必须先做这两步 —— 它们可被证据证伪，不是"我觉得"。**

```text
① 检索：在代码库 / tasks/plan.md / _shared/references/ 里搜关键词
   有现成答案 → 直接用，不讨论。（这是唯一一道真门：可被推翻）
② 判决：把候选方案列成 ≤3 条，逐条写「选它的代价」
   · 代价差 ≤ 一次返工     → 直接选第一条，不讨论
   · 任一条代价不可逆       → 不讨论，走「不可逆操作：先告知人」
   · 三条代价都写不出来     → 说明这是品味问题，直接定
```

> ⚠️ **为什么不用"可逆吗 / 是品味问题吗"这类判据**：
> 它们和"拿不准"**同属自我报告** —— 无法被证伪，因此无法被审计。
> **不可审计的规则在长会话里必然退化成走过场。**
> ① 能被"我搜过了，文件在 X"证伪；② 能被"代价列不出来"证伪。**这才叫门。**
...
- pending 非空 + 无运行中子代理 → **重算可认领数 N**，派 **min(N, 5)** 个
    （可认领 = depends_on 与 stacks_on 全部已在 done/）

【禁止】没有可认领任务却开新一轮 —— 那是空烧轮次。
【收尾】本轮扇出的子代理必须全部 join 或显式处置，否则不得结束本轮。
```

> ⚠️ **objective 写「求值方法 + 上限」，不写「一个数」** ——
> 它是每轮重新注入的文本，N 必须每轮现算。
> 写死"派 1 个"会每轮少派；写死"派全部"会绕过上限 5。
...
#### ⚠️ 补派时机取决于形态（`false` 与 `true` 不同）

**默认 `false` 时，主脑调用是阻塞的** —— 它的下一步只在**本轮全部调用 settle 之后**才发生。
所以"立刻补派"**只在异步 `true` 模式下可执行**：

| 形态 | 本轮动作 | 补派 |
|---|---|---|
| **`false`（默认）** | 同一轮发 `min(N,5)` 个，全部返回后进入下一轮 | **本轮无补派窗口** —— 返回后**按下一轮重新算 N** 继续派 |
| `true`（异步） | 发 N 个后主脑继续干活 | 每个回调到达即补派，保持 `min(5, 可认领数)` |

> ⚠️ **不要因为"本轮无补派窗口"就删掉「分批放行」** —— 它防的是
> **"等一批全做完再开下一批"**（那样最慢的一个会拖住所有槽位）。
> 在 `false` 下正确的做法是：**上一批一返回就立刻开下一批**，
> 而不是等所有任务做完。**这是"轮"的粒度，不是"批"的粒度。**
>
> 队列有 20 个任务、上限 5 → `false` 下是 **4 轮**，每轮 5 个。
> 关键：**每轮结束就开下一轮，不要等 20 个全做完。**
