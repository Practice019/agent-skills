> **来源与本地适配**
> 本文件取自上游仓库 `zhaoxuya520/reverse-skill` 的 `RULES_zh.md`（中文版），拉取于 2026-09-10。
> 本 pack 根目录对应上游仓库的 `skills/`，故原文中所有 `skills/xxx` 路径引用已改写为 `xxx`；独立的 `skills/` 概念名改写为 `reverse-skill-router/`。
> 上游英文版 `RULES.md` 未纳入——本 pack 内全部引用与章节名均为中文（如 `SKILL.md` 引用的「任务完成后的硬性 Checklist」）。
# 逆向/渗透/安全任务自动路由规则

> **本文件是行为链中文副本。** 路由表只在 `config/routing.json`。无论你是什么 AI 编辑器/客户端，读完后必须执行热路径，不要只确认。
...
```
1. NOW:  本文件所在目录即包根
2. NOW:  跑 scripts/master-route.ps1 -Hint "<任务>" → PRIMARY（SSoT: config/routing.json）
...
`reverse-skill-router/`、路由配置、测试、工具清单、case 产物和报告共同构成平台无关核心。Claude Code、Codex、Cursor、OpenCode 或其他 Agent 可以通过各自的项目指令或 skill 适配层加载本仓库，但核心路由与测试不得依赖任何特定客户端文件。
...
1. `scripts/master-route.ps1 -Hint "<任务>"` — 从 `config/routing.json` 选出 PRIMARY
2. `<PRIMARY>/SKILL.md` — 进入目标模块并执行 ACTION REQUIRED
3. `tool-index.md` — 需要本机工具时查询真实状态与路径
4. `routing.md` — 仅在 PRIMARY 歧义时读取的三轴附录，不是第二套路由器

---

## 执行原则

> **决策质量（Issue #77）：** 假设退出、validated 充分性（R4*）、结论锚定与死锁重规划见 ops/analysis-decision-framework.md。**不要**把 R1-R51 全文塞进本文件。
...
- 如果历史方案不适用，在新日志中说明原因
- 检索时按三轴定位：场景类型 / 成功技术 / 目标实体（详见 `field-journal/_index.md` 顶部说明）
...
□ 4. 沉淀搜索到的知识（如果本次任务中联网搜索过）
     ...
     - 如果发现了新场景 → 先更新 tests/routing-benchmark.json，再更新 routing.json；按需同步 MASTER-ROUTING.md 和 routing.md 附录
...
□ 6. 更新系统索引
     ...
     - 检查是否需要更新：routing.json / tests/routing-benchmark.json / MASTER-ROUTING.md / routing.md 附录 / bootstrap-manifest / tool-index
     - 如果发现新工具或新场景 → 执行对应更新
```
...
| 用户确认已手动安装 | 重新运行 refresh-tool-index.ps1（Linux / macOS 用 `bash scripts/refresh-tool-index.sh`）验证，然后继续 |
...
| "我没找到 tool-index，我就直接猜路径" | **缺文件比猜错路径安全。** tool-index 缺失时先运行 `refresh-tool-index.ps1`（Linux / macOS 用 `bash scripts/refresh-tool-index.sh`）生成。猜错路径导致的错误更难排查。 |
...
4. 写入本包对应位置：
   ...
   - 新场景发现 → tests/routing-benchmark.json + routing.json；同步 MASTER-ROUTING.md，必要时补 routing.md 附录
...
1. 先在 `tests/routing-benchmark.json` 添加失败用例
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<本包根目录>/scripts/bootstrap-reverse.ps1" -Capability @('工具名') -StartServices
```

Linux / macOS（Bash）：

```bash
bash <本包根目录>/scripts/bootstrap-reverse.sh 工具名 --start-services
```
...
支持的能力名（与 `scripts/bootstrap-manifest.json` 保持一致，共 26 项）：jadx、apktool、jeb-pro、binaryninja、frida、frida-ps、idalib-mcp、reqable-mcp、jshookmcp、xquik-mcp、anything-analyzer、idapro、r2、rabin2、adb、agent-browser、ghidra-mcp、seclists、proxycat、burpsuite-mcp、nmap、pentestswarm、binwalk、yara、pwntools、bkcrack
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<本包根目录>/scripts/refresh-tool-index.ps1"
```

Linux / macOS（Bash）：

```bash
bash <本包根目录>/scripts/refresh-tool-index.sh
```
...
路径：`<本包根目录>/CONTRIBUTING.md`

新增后必须同步更新：tests/routing-benchmark.json、routing.json、MASTER-ROUTING.md、SKILL.md；涉及工具时再更新 bootstrap-manifest.json、ToolDiscovery.ps1 和 refresh-tool-index.ps1。`routing.md` 仅作为歧义附录按需同步。
...
```
0. NOW: 读 <SKILL_ROOT>/field-journal/precedent-auth.md — 授权预声明（80行）
1. NEXT: 读 <SKILL_ROOT>/MASTER-ROUTING.md 或运行 master-route.ps1 — 定 PRIMARY；疑难再读 routing.md
