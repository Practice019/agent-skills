> 来源：addyosmani/agent-skills v0.6.9 · `performance-optimization.md`（原样搬入，未本地改动）
...
**Do not cache:** anything whose staleness is a correctness bug (balances, permissions, inventory at checkout), or per-user data under a key that does not identify the user. See `../_shared/references/performance-checklist.md` for request coalescing, write strategies, negative caching, and the cache checklist.
...
For detailed performance checklists, optimization commands, and anti-pattern reference, see `../_shared/references/performance-checklist.md`.
