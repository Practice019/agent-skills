...
| --- | --- | --- | --- | --- | --- |
| `weekly_semantic_layer_refresh` | Dedicated projectless thread titled `Semantic Layer Weekly Source Polling`, pinned | `model="gpt-5.5"`, `thinking="xhigh"` when supported | Weekly Monday 9:00 AM local time | Configured `Instructions` from `../plugin-author-config/automation-config.md` | Send one setup kickoff to the pinned thread |
