...
4. 分配 + 扇出    给这 N 个任务逐个标记认领者，**同一轮发 N 个**：
...
7. ★ 重算可认领   本轮标 done 后，**立刻重算可认领集**：
                  有新增解锁的 → **本轮直接追加派发**，不要等下一轮
                  仍不足 5 个且还有可认领 → 继续补派
8. 评审           命中触发条件（不可逆/安全/接口/需评审/有不确定）的任务：
                  任务移入 review/，派**另一个**子代理评审（见「评审门禁」）
                  不通过 → 回退 pending/ 带问题重派
9. 交棒           汇总产物、更新 goal 状态、决定下一轮
```

> ★ **第 7 步是依赖任务的关键。** 一轮派 N 个 → 其中一个完成 → 它解锁了下游任务
