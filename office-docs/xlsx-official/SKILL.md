...
> **DSH environment note:** the original text targets MiMo's bundled runtime (`MIMO_PYTHON` / `MIMO_SOFFICE`). DSH has no such runtime — use the system `python` (Python 3.13.5 with openpyxl/pandas/lxml/xlsxwriter already installed) or `uv run` instead. Replace every `"$MIMO_PYTHON"` with `python`, and install LibreOffice yourself if `soffice` is needed.
